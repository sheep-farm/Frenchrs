...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Retornos mensais simulados de um fundo
    let fund_returns = array![
        0.058, 0.042, -0.018, 0.082, 0.045, -0.032, 0.068, 0.052, -0.015, 0.062, 0.041, 0.075
    ];

    let rf = 0.03 / 12.0; // Taxa livre de risco mensal

    // ========================================================================
    // EXEMPLO 1: APT com 2 fatores (Mercado e Momentum)
    // ========================================================================
    println!("\n{}", "-".repeat(80));
    println!("EXEMPLO 1: APT com 2 Fatores (Mercado + Momentum)");
    println!("{}", "-".repeat(80));
    ...
    let factor_names_2 = Some(vec!["Mercado".to_string(), "Momentum".to_string()]);
    ...
    // ========================================================================
    // EXEMPLO 2: APT com 4 fatores (Mercado, Tamanho, Valor, Momentum)
    // ========================================================================
    println!("\n{}", "-".repeat(80));
    println!("EXEMPLO 2: APT com 4 Fatores (Mercado + SMB + HML + Momentum)");
    println!("{}", "-".repeat(80));

    let factors_4 = Array2::from_shape_vec(
        (12, 4),
        vec![
            // Mercado, SMB, HML, Momentum
    ...
    // ========================================================================
    // EXEMPLO 3: APT com 6 fatores customizados
    // ========================================================================
    println!("\n{}", "-".repeat(80));
    println!("EXEMPLO 3: APT com 6 Fatores Customizados");
    println!("{}", "-".repeat(80));

    let factors_6 = Array2::from_shape_vec(
        (12, 6),
        vec![
            // Mercado, SMB, HML, RMW, CMA, Momentum
    ...
    println!("{}", "=".repeat(80));
    println!("\n{:<30} {:>12} {:>12}", "Modelo", "R²", "R² Ajustado");
    println!("{}", "-".repeat(80));
    println!(
        "{:<30} {:>12.4} {:>12.4}",
        "APT-2 (Mercado + Mom)", apt2.r_squared, apt2.adj_r_squared
    );
    println!(
        "{:<30} {:>12.4} {:>12.4}  [+{:.4}]",
        "APT-4 (+ SMB + HML)",
    ...
    // ========================================================================
    println!("\n{}", "=".repeat(80));
    println!("ANÁLISE DE SIGNIFICÂNCIA DOS FATORES (α = 0.05)");
    println!("{}", "=".repeat(80));

    println!("\nAPT-6 Fatores:");
    let factor_labels = vec!["Mercado", "SMB", "HML", "RMW", "CMA", "Momentum"];
    ...
    if apt6.is_significantly_outperforming(0.05) {
        println!("\n>>> O fundo está gerando ALPHA positivo significativo!");
    } else if apt6.alpha > 0.0 {
        println!("\n>>> O fundo tem alpha positivo, mas não é estatisticamente significante.");
    } else {
        println!("\n>>> O fundo não está gerando alpha positivo.");
    }
